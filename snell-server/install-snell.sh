...
migrate_legacy_config() {
    ...
    local backup_file="${config_file}.v5.bak"
    local migrated_file

    if ! grep -Eq '^[[:space:]]*(obfs|ipv6)[[:space:]]*=' "$config_file"; then
        return
    fi
    ...
    # Convert the options generated by older revisions while preserving port and PSK.
    migrated_file=$(mktemp "${config_file}.XXXXXX")
    if ! sed -E \
        -e '/^[[:space:]]*obfs[[:space:]]*=/d' \
        -e '/^[[:space:]]*ipv6[[:space:]]*=/d' \
        -e 's/^([[:space:]]*listen[[:space:]]*=[[:space:]]*0\.0\.0\.0:([0-9]+))[[:space:]]*$/\1,[::]:\2/' \
        "$config_file" > "$migrated_file"; then
        rm -f "$migrated_file"
        print_status "error" "旧版配置迁移失败"
        return 1
    fi

    grep -Eq '^[[:space:]]*mode[[:space:]]*=' "$migrated_file" || echo 'mode = default' >> "$migrated_file"
    grep -Eq '^[[:space:]]*dns-ip-preference[[:space:]]*=' "$migrated_file" || echo 'dns-ip-preference = default' >> "$migrated_file"
    mv "$migrated_file" "$config_file"

    print_status "success" "旧版配置已迁移到 Snell v6，备份位于 ${backup_file}"
}
...
print_service_details() {
    if systemctl is-active --quiet ${SERVICE_NAME}; then
        local config_file="${CONFIG_DIR}/config.conf"
        local port=$(sed -nE 's/^[[:space:]]*listen[[:space:]]*=[[:space:]]*[^,]*:([0-9]+).*/\1/p' "$config_file" | head -n 1)
